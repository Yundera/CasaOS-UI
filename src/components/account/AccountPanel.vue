    ...
    <ValidationObserver ref="observer" v-slot="{ handleSubmit }">
      ...
      <section class="modal-card-body " :class="bodyPadding">
        ...
        <template v-else-if="state == 3">
          ...
          <ValidationProvider v-slot="{ errors, valid }" name="oriPassword" rules="required" vid="oriPassword">
          ...
          <ValidationProvider v-slot="{ errors, valid }" name="Password" rules="required|password_policy" vid="password">
            <b-field :message="$t(errors)" :type="{ 'is-danger': errors[0], 'is-success': valid }" class="mb-5 has-text-light">
          ...
          <ValidationProvider v-slot="{ errors, valid }" name="Password Confirmation" rules="required|confirmed:password">
            <b-field :message="$t(errors)" :type="{ 'is-danger': errors[0], 'is-success': valid }" class="mb-0">
              <b-input v-model="confirmation" :placeholder="$t('Confirm the new password again')" password-reveal type="password" v-on:keyup.enter.native="savePassword(savePassword)"></b-input>
            </b-field>
          </ValidationProvider>
        </template>
        ...
        <template v-else-if="state == 4">
          <div class="is-flex">
            <div class="cropper-wrapper is-flex-grow-0 is-flex-shrink-0">
              <cropper :src="image.src" @change="onChange" :debounce="false" :stencil-props="stencilProps" check-orientation :min-height="80" :min-width="80" :canvas="canvasProps" :default-size="defaultSize" />
            </div>
